...
WHERE trip_time_in_secs >= 60 OR
	  trip_distance < 0.5 OR
	  ((trip_distance * 1.609 * 1000) / trip_time_in_secs) * 3.6 > 70 OR
	  passenger_count BETWEEN 1 AND 6 OR
	  d.pickup_datetime ISNULL OR
	  d.dropoff_datetime ISNULL OR
...
COPY ( SELECT * FROM driver_analysis ) TO 'D:/driver_analysis.csv' with CSV DELIMITER ',' HEADER;
